...
import { UsersThree, FolderOpen, MagnifyingGlass, Bug, PlusCircle, Bell, UserCircle, Sparkle, Clock, CheckCircle } from 'phosphor-react'
...
const TIPO_ICON = { similitud: <MagnifyingGlass size={16} />, revision: <CheckCircle size={16} />, mensaje: <Bell size={16} />, sistema: <Sparkle size={16} /> }
    ...
    <DashboardLayout role="admin" titulo="Dashboard">
      <div className={s.page}>
        <section className={s.hero}>
          ...
          <p className={s.heroText}>Monitorea usuarios, proyectos, similitudes detectadas y reportes de fallas de toda la plataforma ProyecTwin.</p>
        </section>

        <section className={s.stats} aria-label="Estadísticas generales">
          <Link to="/admin/gestion-usuarios" className={s.statLink}><MetricCard icon={<UsersThree size={22} />} label="Usuarios" value={datos.totalUsuarios} variant="primary" trend={`${datos.aprendices} AP · ${datos.instructores} IN`} /></Link>
          <Link to="/admin/proyectos" className={s.statLink}><MetricCard icon={<FolderOpen size={22} />} label="Proyectos" value={datos.totalProyectos} variant="success" trend={`${datos.pendientes} pendientes`} /></Link>
        ...
        <DataPanel title="Acciones rápidas" icon={<Sparkle size={18} />}>
          <div className={s.quick}>
            <Link to="/admin/nuevo-usuario" className={s.quickItem}><PlusCircle size={18} /> Crear usuario</Link>
            <Link to="/admin/gestion-usuarios" className={s.quickItem}><UsersThree size={18} /> Gestionar usuarios</Link>
            <Link to="/admin/proyectos" className={s.quickItem}><FolderOpen size={18} /> Ver proyectos</Link>
            <Link to="/admin/similitudes" className={s.quickItem}><MagnifyingGlass size={18} /> Revisar similitudes</Link>
            <Link to="/admin/reportes-fallas" className={s.quickItem}><Bug size={18} /> Reportes de fallas</Link>
            <Link to="/admin/perfil" className={s.quickItem}><UserCircle size={18} /> Mi perfil</Link>
          </div>
        </DataPanel>
